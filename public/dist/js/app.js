...
function blankSvg() {
  ...
  that.render = function (appendTo) {
    var svg = d3.select(appendTo).append('svg').attr('height', '500').attr('width', '500');
  };
...
var svg = d3.select("body").append("svg").attr("width", width).attr("height", height).append("g").attr("class", "zoom-container").call(d3.behavior.zoom().x(x).y(y).scale(1).scaleExtent([1, 1]).on("zoom", zoom));

var rect = svg.append("rect").attr("class", "overlay").attr("width", width).attr("height", height);

var circle = svg.selectAll("circle").data(data).enter().append("circle").attr("r", radius).attr("transform", transform);

var drone = svg.selectAll(".drone").data([width / 2, height / 2]).enter().append("image").attr("transform", "translate(" + (width / 2 - droneWidth / 2) + "," + (height / 2 - droneHeight / 2) + ")").attr("xlink:href", "./css/images/drone.png").attr("height", droneHeight + "px").attr("width", droneWidth + "px");
...
function zoom() {
  ...
  // red dots
  var container = svg.selectAll("g").data(inrange).enter().append("g").attr("transform", transform);

  container.append("ellipse").attr("rx", function (d) {
    return Math.min(2000 / (d[2] + 1), radius);
  }).attr("ry", function (d) {
    return Math.min(2000 / (d[2] + 1), radius);
  }).style("fill", "red").style("fill-opacity", .7);

  container.append("text").attr("x", function (d) {
    return -1 * Math.min(1000 / (d[2] + 1), 7);
  }).attr("y", function (d) {
    return Math.min(700 / (d[2] + 1), 5);
  }).attr("font-size", function (d) {
    return Math.min(2000 / (d[2] + 1), 17);
  }).text(function (d) {
    return parseInt(d[2]);
...
function findRegion(point, box) {
  var x = point[0],
      ...
      y1 = box[1][1];
  if (y >= y1) {
    if (x >= x1) return 'UpperLeft';else if (x <= x0) return 'UpperRight';else return 'Up';
  } else if (y <= y0) {
    if (x >= x1) return 'LowerLeft';else if (x <= x0) return 'LowerRight';else return 'Low';
  } else {
    if (x >= x1) return 'Left';else if (x <= x0) return 'Right';
  }
  return "unknown";
}
